...
def make_splits_dfdc(dfdc_df_path: str):
    df = pd.read_pickle(dfdc_df_path)

    # df_train = df[df['folder'].isin(range(1))]
    # df_val = df[df['folder'].isin(range(1, 2))]
    random_true_videos = np.random.permutation(
        df[(df['label'] != True)]['video'].unique())
    train_orig = random_true_videos[:300]
    val_orig = random_true_videos[300:320]
    df_train = pd.concat(
        (df[df['original'].isin(train_orig)], df[df['video'].isin(train_orig)]), axis=0)
    df_val = pd.concat(
        (df[df['original'].isin(val_orig)], df[df['video'].isin(val_orig)]), axis=0)
