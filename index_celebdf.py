...
def main():
    ...
    parser.add_argument('--source', type=Path, help='Source dir',
                        required=True)
    parser.add_argument('--videodataset', type=Path, default='/mnt/8T/hou/deepfake_dataset/Celeb-DF/celebdf_videos.pkl',
    ...
    if videodataset_path.exists():
        ...
        if not split_file.exists():
            raise FileNotFoundError(
                'Unable to find "List_of_testing_videos.txt" in {}'.format(source_dir))
        test_videos_df = pd.read_csv(
            split_file, delimiter=' ', header=0, index_col=1)
        ...
        df_videos['test'] = df_videos['path'].map(
            str).isin(test_videos_df.index)
