...
from toolkits.utils import extract_meta_av
...
def main(argv):
    ...
    if videodataset_path.exists():
        ...
        for idx, json_path in enumerate(tqdm(sorted(source_dir.rglob('metadata.json')), desc='Indexing')):
            ...
            ##df_tmp['folder'] = str(json_path.parts[-2]).split('_')[-1]
        ...
        ##df_videos['folder'] = df_videos['folder']
        df_videos['folder'] = df_videos['folder'].astype(np.uint8)

        # Collect metadata
        paths_arr = np.asarray(df_videos.path.map(
        ...
        with Pool() as pool:
            for batch_idx0 in tqdm(np.arange(start=0, stop=len(df_videos), step=batch_size), desc='Metadata'):
                batch_res = pool.map(
                    extract_meta_av, paths_arr[batch_idx0:batch_idx0 + batch_size])
